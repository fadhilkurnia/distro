...
TEMP_NAME = "batch"
REPO = "https://github.com/PanjiSri/xdn.git"
COMMIT_HASH = "e1a976845b4be5994c0938796b3341e6bc1e3f21"
...
SERVICE_NAME = "todo"
CONSISTENCY = [{"num": 1, "text": "Linearizability",    "deterministic": f"{SERVICE_NAME}-d-linearizability.yaml",   "non-deterministic": None},
               {"num": 2, "text": "Sequential",         "deterministic": f"{SERVICE_NAME}-d-sequential.yaml",        "non-deterministic": None},
               {"num": 3, "text": "Causal",             "deterministic": f"{SERVICE_NAME}-d-causal.yaml",            "non-deterministic": None},
               {"num": 4, "text": "PRAM",               "deterministic": f"{SERVICE_NAME}-d-pram.yaml",              "non-deterministic": None},
               {"num": 5, "text": "Eventual",           "deterministic": f"{SERVICE_NAME}-d-eventual.yaml",          "non-deterministic": None}]
...
class XdnLauncher(Launcher):
    ...
    def start(self, nodes_map):
        ...
        fuselog = f"{self.repo_dir_path}/bin/fuselog"
        fuselog_apply = f"{self.local_dir}/bin/fuselog-apply"
    ...
    def stop(self, nodes_map):
        for i, node in enumerate(nodes_map):
            core_cleanup_cmd = (
                ...
                f"container_ids=$(docker ps -a -q --filter 'name=c0.e0.{SERVICE_NAME}.ar{i}.xdn.io'); "
                ...
                f"for mountpoint in $(find /tmp/xdn/state/fuselog/ -type d -name 'ar{i}' | xargs -I{{}} echo {{}}/mnt/{SERVICE_NAME}/e0); do "
