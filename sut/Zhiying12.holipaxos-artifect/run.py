...
REPO = "https://github.com/PanjiSri/holipaxos-artifect"
...
def main(run_ycsb, nodes, ssh) -> None:
    ...
    while True:
        ...
        match val:
            ...
            case 2:
                endpoints = [f"{node["private_ip"]}:{node["client_port"]}" for node in port_map]
                run_ycsb(selected_protocol, "holipaxos", [",".join(endpoints)], "holipaxos.hosts", ssh)
...
def start(protocol, ssh, ip_port_map):
    ...
    for i, node in enumerate(ip_port_map):
        if node["private_ip"] == "127.0.0.1" and node["public_ip"] == "127.0.0.1":
            local_start(binary_path, protocol, config_path, i)
        else:
            host = node["public_ip"]
            remote_start(binary_path, protocol, config_path, i, host, ssh)

    print(f"{protocol["name"]} instances successfully started")
...
def stop(protocol, ssh, ip_port_map):
    for i, node in enumerate(ip_port_map):
        if node["private_ip"] == "127.0.0.1" and node["public_ip"] == "127.0.0.1":
            ...
            host = node["public_ip"]
            remote_stop(protocol, host, ssh)

    print("Removing local run_config.json")
    config = CURR_DIR / "run_config.json"
    subprocess.run(f"rm {config.resolve()}", shell=True, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def generate_config(port_map, persistency):
    print("Generating run_config.json file")
...
def local_start(binary, protocol, config_path, id):
    if protocol["name"] == "omnipaxos":
        run_cmd = f"nohup {binary.resolve()} --id {id} --config-path {config_path.resolve()} > /dev/null 2>&1 &"
    else:
        run_cmd = f"nohup {binary.resolve()} -id {id} -c {config_path.resolve()} -d > /dev/null 2>&1 &"

    print(f"Starting {protocol["name"]} on localhost")
    subprocess.run(run_cmd, check=True, shell=True)


def remote_start(binary, protocol, config_path, id, host, ssh):
    user = ssh["username"]
    ...
    print(f"Sending {protocol["name"]} binary to {host}")
    subprocess.run(copy_cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    print(f"Starting {protocol["name"]} remotely on {host}")
    subprocess.run(run_cmd, check=True, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def local_stop(protocol):
    binary_path = BIN_DIR / protocol["binary"]
    cmd = (
        f"pids=$(ps aux | grep '{binary_path.resolve()}' | grep -v grep | awk '{{print $2}}'); "
            f"for pid in $pids; do echo \"Killing $pid\"; kill -9 $pid; done; "
    )
    print(f"Stopping local {protocol["name"]} instance")
    subprocess.run(cmd, shell=True, check=True)
    subprocess.run("rm server.*.log", shell=True, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def remote_stop(protocol, host, ssh):
    user = ssh["username"]
    remote_dir = f"/home/{user}/holipaxos"
    remote_binary = f"{remote_dir}/{protocol["binary"]}"
    remote_config = f"{remote_dir}/run_config.json"
    ...
    cmd = ["ssh", "-i", str(ssh["key"]), f"{user}@{host}",
           remote_command]
    print(f"Stopping remote {protocol["name"]} instance on {host}")
